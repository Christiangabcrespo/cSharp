...
namespace Ejercicio09
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Espectador per2 = new Espectador("Laura", 24, 9999);
            Espectador per3 = new Espectador("Pedro", 10, 50);
            Espectador per4 = new Espectador("Eufrasio", 69, 5);
            espectados.Add(per1);
            espectados.Add(per2);
            espectados.Add(per3);
            espectados.Add(per4);
            ...
            foreach (Espectador espectador in espectados)
            {
                if (espectador.Edad > cine.Peli.Edad)
                {
                    if (espectador.Dinero >= cine.Precio)
                    {
                        ...
                        Console.WriteLine("El asiento {0}",sala.AsientoOcupados[cont]);
                        Console.WriteLine("");
                        cont++;
                    }
                    else
                    {
                        Console.WriteLine("{0} no tiene el dinero suficiente",espectador.Nombre);
                        Console.WriteLine("");
                    }
                }
                else
